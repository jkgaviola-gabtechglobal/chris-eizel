"use client";

import Image from "next/image";
import { motion } from "framer-motion";
import type { Variants } from "framer-motion";

const EASE = [0.22, 1, 0.36, 1] as const;

const fadeUp: Variants = {
  hidden: { opacity: 0, y: 28 },
  show: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { delay: i * 0.13, duration: 0.65, ease: EASE },
  }),
};

const listItem: Variants = {
  hidden: { opacity: 0, x: -18 },
  show: (i: number) => ({
    opacity: 1,
    x: 0,
    transition: { delay: i * 0.1, duration: 0.5, ease: EASE },
  }),
};

const bullets = [
  "High school diplomas",
  "University or college degrees",
  "Accredited online programs",
  "Career-focused training and certifications",
];

export default function About() {
  return (
    <section id="about" className="bg-[#f0f6fb] pt-20 pb-0 px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto">

        {/* Header */}
        <div className="mb-10">
          <motion.p
            className="text-[#2AACE2] font-bold tracking-widest uppercase text-sm mb-3 text-center"
            custom={0}
            variants={fadeUp}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-60px" }}
          >
            About Chris &amp; Eizel Foundation
          </motion.p>

          <motion.h2
            className="text-4xl md:text-5xl font-black text-gray-900 mb-6 text-center"
            custom={1}
            variants={fadeUp}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-60px" }}
          >
            Building Pathways. Changing Lives.
          </motion.h2>

          <motion.p
            className="text-gray-700 font-semibold text-base md:text-lg text-left"
            custom={2}
            variants={fadeUp}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-60px" }}
          >
            The Chris &amp; Eizel Foundation was established with a simple but powerful belief: education changes the trajectory of a life.
          </motion.p>

          <motion.p
            className="text-gray-700 text-base md:text-lg leading-relaxed mt-4 text-left"
            custom={3}
            variants={fadeUp}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, margin: "-60px" }}
          >
            We exist to provide youth—especially those facing adversity—with access to education, training, and career pathways that open doors to a better future. Many young people have the potential to succeed but lack the financial resources, mentorship, or support systems needed to move forward. Our foundation was created to help bridge that gap.
          </motion.p>
        </div>

        {/* Two-column layout */}
        <div className="flex flex-col md:flex-row gap-6 md:gap-0 items-start">

          {/* Left: Image — slides in from left */}
          <motion.div
            className="w-full md:w-3/5 shrink-0 -ml-6 md:-ml-28"
            initial={{ opacity: 0, x: -60 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-80px" }}
            transition={{ duration: 0.85, ease: EASE }}
          >
            <div className="relative w-full h-96 sm:h-120 md:h-150 rounded-3xl overflow-hidden">
              <Image
                src="/Image2.png"
                alt="Chris and Eizel - Founders of Chris & Eizel Foundation"
                fill
                sizes="(max-width: 768px) 100vw, 40vw"
                loading="eager"
                className="object-cover object-top"
                style={{ objectPosition: "20% 30%" }}
              />
            </div>
          </motion.div>

          {/* Right: Text — slides in from right */}
          <motion.div
            className="w-full md:w-3/5 flex flex-col gap-5 text-gray-700 text-base md:text-lg leading-relaxed md:-ml-16"
            initial={{ opacity: 0, x: 60 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, margin: "-80px" }}
            transition={{ duration: 0.85, ease: EASE, delay: 0.15 }}
          >
            <div>
              <motion.p
                className="font-semibold text-gray-900 mb-3"
                initial={{ opacity: 0 }}
                whileInView={{ opacity: 1 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: 0.3 }}
              >
                Through strategic fundraising and community partnerships, we provide scholarships and financial assistance to at-risk youth pursuing:
              </motion.p>

              <ul className="space-y-2">
                {bullets.map((item, i) => (
                  <motion.li
                    key={item}
                    className="flex items-center gap-3"
                    custom={i}
                    variants={listItem}
                    initial="hidden"
                    whileInView="show"
                    viewport={{ once: true }}
                  >
                    <motion.span
                      className="w-2 h-2 rounded-full bg-[#2AACE2] shrink-0"
                      initial={{ scale: 0 }}
                      whileInView={{ scale: 1 }}
                      viewport={{ once: true }}
                      transition={{ delay: 0.35 + i * 0.1, duration: 0.35, ease: EASE }}
                    />
                    {item}
                  </motion.li>
                ))}
              </ul>
            </div>

            <motion.p
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.2, ease: EASE }}
            >
              Our focus is not just on funding education, but on empowering transformation. We believe that when a young person gains access to opportunity, they gain confidence, dignity, and hope. And when one life changes, families and communities change with it.
            </motion.p>

            <motion.p
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.3, ease: EASE }}
            >
              Chris and Eizel founded this organization with deep conviction that no young person&apos;s future should be limited by circumstance. Education is more than academics—it is stability, direction, and possibility.
            </motion.p>

            {/* Closing statement with animated underline */}
            <motion.div
              className="relative"
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.4, ease: EASE }}
            >
              <p className="font-semibold text-[#2AACE2]">
                We are committed to building pathways where there were once barriers. We are committed to investing in potential. We are committed to hope in action.
              </p>
              <motion.span
                className="block h-0.5 bg-[#2AACE2] rounded-full mt-2"
                initial={{ scaleX: 0, originX: 0 }}
                whileInView={{ scaleX: 1 }}
                viewport={{ once: true }}
                transition={{ delay: 0.6, duration: 0.7, ease: EASE }}
              />
            </motion.div>

          </motion.div>
        </div>
      </div>
    </section>
  );
}
